//* Esse arquivo acho que trabalha junto com o crawler, pra importar personagens do DnD Beyond
...
class DnDBeyondImporter {
    getCharacter(url, callback) {
        ...
        try {
            ...
            xhttp.onreadystatechange = function () {
                ...
                allModifiers = allModifiers.concat(response.modifiers.class).concat(response.modifiers.feat).concat(response.modifiers.item).concat(response.modifiers.background).concat(response.modifiers.condition);
                ...
                var hpPerLevel = allModifiers
                    .filter((x) => x.subType == "hit-points-per-level")
                    .map((x) => x.value)
                    .reduce((a, b) => a + b, 0);
                var conBonus = allModifiers
                    .filter((x) => x.subType == "constitution-score")
                    .map((x) => x.value)
                    .reduce((a, b) => a + b, 0);
                var wisBonus = allModifiers
                    .filter((x) => x.subType == "wisdom-score")
                    .map((x) => x.value)
                    .reduce((a, b) => a + b, 0);
                var dexBonus = allModifiers
                    .filter((x) => x.subType == "dexterity-score")
                    .map((x) => x.value)
                    .reduce((a, b) => a + b, 0);

                var darkvision = allModifiers.find((x) => x.type == "set-base" && x.subType == "darkvision");
                if (darkvision) returnObj.darkvision = darkvision.value;
    ...
    setArmorClass(response, returnObj, allModifiers) {
                ...
                function getRelevantModifer(unarmoredDef) {
                    switch (unarmoredDef.statId) {
                        case null:
                            return null;
                        case 3:
                            return returnObj.constitution;
                        case 5:
                            return returnObj.wisdom;
                    }
                }
            }
            returnObj.ac = baseAc;
        ...
        function getItemTrueAc(eq) {
            ...
            if (acBonusFromArmor != null) eq.definition.armorClass += acBonusFromArmor.value;
