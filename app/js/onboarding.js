...
class Onboarding {
    ...
    init() {
        ...
        dataAccess.getSettings((settings) => {
            if (settings.onboardingLast && this.versionHigherThan(settings.onboardingLast, compareVersion)) return;
            if (thisVersion) {
    ...
    baseCreate(modalContents) {
        ...
        modalContents.paragraphs.forEach((x) => {
            if (typeof x == "string") return pCont.appendChild(util.ele("p", "text_left", x));
            if (x.type == "img") {
    ...
    display(modalContents) {
        ...
        if (modalContents.width) modal.modal.style.maxWidth = modalContents.width;
