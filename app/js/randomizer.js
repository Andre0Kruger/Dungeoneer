var randomizer = (function () {
    ...
    function go() {
        ...
        if (searchSet.substring(0, 9) == "Encounter") return generateRandomEncounter(searchSet);
    ...
    function generateRandomEncounter(searchString) {
        ...
        if (difficulty == "any")
            difficulty = [...randomizerEncounterDangerSelect.options]
                .map((x) => x.value)
                .filter((x) => x != "any")
                .pickOne();

        if (selectedNumber == "any")
            selectedNumber = [...randomizerEncounterCreatureNumberSelect.options]
                .map((x) => x.value)
                .filter((x) => x != "any")
                .pickOne();
        ...
        if (filterTypes) forcedType = searchSet != "Any" ? searchSet : null;
        if (filterTags) monsterTags = searchSet;
    ...
    function filterRarity(data, selectedRarity) {
        data = data.filter(function (entry) {
            return entry.rarity != null && entry.rarity.toLowerCase() == selectedRarity.toLowerCase();
        });
    ...
    function initialize() {
        dataAccess.getRandomTables(function (data) {
            ...
            dataAccess.getItems(function (itemData) {
                itemTypeValues.forEach((itemType) => {
                    if (itemData.filter((x) => x.type.toLowerCase() == itemType.toLowerCase()) != 0) values.push(itemType);
                });
                ...
                generateEncounterLookupValues(obj.encounter_sets, function (encounterValues) {
                    ...
                    new Awesomplete(randomizerInput, {
                        list: values,
                        autoFirst: true,
                        minChars: 0,
                        maxItems: 99,
        ...
        randomizerRaritySelect.onchange = function (evt) {
            go();
        };
        randomizerInput.addEventListener("keydown", function (evt) {
            if (evt.keyCode == 13) go();
        });
        randomizerEncounterCreatureNumberSelect.onchange = function (evt) {
            go();
        };
        randomizerEncounterDangerSelect.onchange = function (evt) {
            go();
        };
    }
        ...
        dataAccess.getMonsters((monsters) => {
            dataAccess.getHomebrewMonsters((hbMonsters) => {
                dataAccess.getTags((tags) => {
                    ...
                    tags.forEach((tag) => {
                        allValues.push("Encounter(tag): " + tag);
                    });
