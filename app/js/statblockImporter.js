...
class Importer {
    ...
    importSchema1(e) {
        ...
        if (typeAndSubtype.subtype) newObj.subtype = typeAndSubtype.subtype;
        if (e.alignment) {
        ...
        ["strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma"].forEach((prop) => {
            if (e.save && e.save[prop.substring(0, 3)]) newObj[prop + "_save"] = e.save[prop.substring(0, 3)];
        });
        if (e.senses) newObj.senses = e.senses.join(", ");

        if (e.languages) newObj.languages = e.languages.join(", ");
        ...
        if (e.conditionImmune) newObj.condition_immunities = e.conditionImmune.join(", ");
        ...
        e.action?.forEach((action) => {
            ...
            if (damageDiceString) newAction.damage_dice = damageDiceString.trim();
            if (damageBonusTotal > 0) newAction.damage_bonus = damageBonusTotal;
        ...
        function getSpecialAbilities(e) {
            ...
            if (e.spellcasting) {
                ...
                e.spellcasting.forEach((spellcasting) => {
                    if (spellcasting.headerEntries) str += spellcasting.headerEntries.join("\n\n");
        ...
        function getSpeed(speed) {
            var base = "";
            if (speed.walk) base += speed.walk + " ft";
            if (speed.climb) {
                base += base.length == 0 ? "" : ", ";
        ...
        function getType(type) {
            var obj = {};
            if (type.type) {
                if (type.tags) obj.subtype = type.tags[0];
                obj.type = type.type;
                return obj;
            }
            obj.type = type;
...
String.prototype.replacePlaceHolders = function () {
    return this.replaceAll("{@hit", "").replaceAll("{@damage", "").replaceAll("}", "").replaceAll("{@condition ", "").replaceAll("{@atk mw  ", "+").replaceAll("{@dc", "dc").replaceAll("{@dice", "").replaceAll("{@h", "").replaceAll("{@recharge", "Recharge");
};
