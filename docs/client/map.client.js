...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (gridEnable != settings.enableGrid + "") toggleGrid();
...
function createPerspectiveDropdown() {
    ...
    if (TOKEN_ACCESS == null) return;

    TOKEN_ACCESS.forEach((pawn) => {
        var opt = createOption(pawn.character_name, pawn.character_name);
        if ((selected && selected == pawn.character_name) || TOKEN_ACCESS.length == 1) opt.selected = true;
        dd.appendChild(opt);
    });
