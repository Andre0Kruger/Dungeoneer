class SoundManager {
    ...
    importSound(libraryEntry) {
        if (this.IMPORTED_LIBRARY.find((x) => x.name.toLowerCase() == libraryEntry.name.toLowerCase())) return;
    ...
    checkQueue() {
        ...
        this.createCustomSoundQueue.forEach(async (queuedSound) => {
            if (await cls.getSoundInfo(queuedSound.effect.src)) await cls.addEffect(queuedSound.effect, queuedSound.elementId);
        });
    }
    ...
    async addEffect(effect, elementId) {
        ...
        if (!info) {
            ...
            this.createCustomSoundQueue.push({
                effect: effect,
                elementId: elementId,
            });
            return;
