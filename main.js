...
function getWindow(windowName) {
    switch (windowName) {
        ...
        case "walkCalculatorWindow":
            return walkCalculatorWindow;
...
ipcMain.on("open-walk-calculator-window", function () {
    if (walkCalculatorWindow) {
        walkCalculatorWindow.focus();
        return;
    }

    walkCalculatorWindow = createBaseWindow({ title: "Walk Calculator" }, "/app/walk-calculator.html", () => {});

    walkCalculatorWindow.on("closed", function () {
        walkCalculatorWindow = null;
    });
});
